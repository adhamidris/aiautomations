...
export function TrustedMarquee() {
  return (
    <section className="w-full border-y border-white/10 bg-black/40 backdrop-blur-sm overflow-hidden py-6 relative">
          ...
          >
            {MARQUEE_CONTENT.map((company, i) => (
              <div key={i} className="flex items-center gap-2 group cursor-default">
                {/* Placeholder Icon */}
                <div className="w-2 h-2 rounded-sm bg-white/40 rotate-45 group-hover:bg-accent transition-colors duration-300" />
                <span className="text-sm font-heading font-bold text-white/70 tracking-widest group-hover:text-white transition-colors duration-300">
                  {company}
                </span>
              </div>
            ))}
